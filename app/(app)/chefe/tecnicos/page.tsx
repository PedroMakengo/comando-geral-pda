...
interface AuthUser {
  ...
  chefeDe?: { id: string; nome: string } | null
...
export default function AvaliarTecnicosChefePage() {
  ...
  const [sheetTecnico, setSheetTecnico] = useState(false)
  const [tecnicoVer, setTecnicoVer] = useState<Tecnico | null>(null)

  const [sheetAvaliar, setSheetAvaliar] = useState(false)
  const [tecnicoAvaliar, setTecnicoAvaliar] = useState<Tecnico | null>(null)
  ...
  // Departamento do chefe — chefeDe tem prioridade sobre departamento
  const deptId =
    user?.chefeDe?.id ?? user?.departamento?.id ?? user?.departamentoId
  const deptNome = user?.chefeDe?.nome ?? user?.departamento?.nome ?? '—'

  const fetchTecnicos = useCallback(async () => {
    if (!deptId) return
    setLoading(true)
    try {
      // 1. Buscar técnicos do departamento
      const params = new URLSearchParams({
        page: String(page),
        limit: '50', // buscar mais para ter todos
        departamentoId: deptId,
        role: 'Tecnico',
        ...(search && { search }),
      })
      const data = await safeFetch(`/api/utilizadores?${params}`)
      const todosTecnicos: Tecnico[] = toArray<Tecnico>(data).filter(
        (t: any) => t.id !== user?.id,
      )
      ...
      // 2. Buscar TODAS as fichas do departamento de uma vez (mais eficiente)
      const fichasData = await safeFetch(
        `/api/fichas?departamentoId=${deptId}&limit=200`,
      )
      const todasFichas: FichaActual[] = toArray<any>(fichasData).map(
        (f: any) => ({
          id: f.id,
          estado: f.estado,
          periodo: f.periodo,
      ...
      // 3. Associar ficha mais recente a cada técnico
      ...
      const comFichas = todosTecnicos.map((t) => {
        const fichasDoTecnico = todasFichas
          .filter((f: any) => f.avaliadoId === t.id)
          .sort(
            (a: any, b: any) =>
              (prioridade[b.estado] ?? 0) - (prioridade[a.estado] ?? 0),
          )
        return { ...t, fichaActual: fichasDoTecnico[0] ?? null }
      })

      setTecnicos(comFichas)
  ...
  }, [deptId, page, search, user?.id])

  useEffect(() => {
    if (ready) fetchTecnicos()
  }, [fetchTecnicos, ready])
  ...
  const verTecnico = (t: Tecnico) => {
    setTecnicoVer(t)
    setSheetTecnico(true)
  }

  const abrirAvaliar = async (t: Tecnico) => {
    if (!t.fichaActual) {
      toast.error('Este técnico não tem ficha de avaliação.')
      return
    }
    if (t.fichaActual.estado !== 'Pendente') {
      toast.error(
        `Este técnico já foi avaliado (${estadoCfg[t.fichaActual.estado]?.label ?? t.fichaActual.estado}).`,
      )
      return
    }
    setTecnicoAvaliar(t)
  ...
  const submeterAvaliacao = async () => {
    if (!tecnicoAvaliar?.fichaActual || !todosAvaliados) {
    ...
    try {
      const res = await fetch('/api/submissoes', {
        ...
        credentials: 'include',
        body: JSON.stringify({
          fichaId: tecnicoAvaliar.fichaActual.id,
      ...
      toast.success(`Avaliação de ${tecnicoAvaliar.nomeCompleto} submetida!`, {
        description: 'Aguarda validação do Director.',
      })
  ...
  const tecnicosFiltrados = search
    ? tecnicos.filter((t) =>
        t.nomeCompleto.toLowerCase().includes(search.toLowerCase()),
      )
    : tecnicos

  const paraAvaliar = tecnicos.filter(
    (t) => t.fichaActual?.estado === 'Pendente',
  ).length
    ...
    return (
      <div className="rounded-xl border border-dashed border-zinc-300 bg-zinc-50 p-14 text-center">
        ...
        <p className="text-xs text-zinc-400 mt-1">
          O seu utilizador não tem departamento nem departamento liderado.
          <br />
          Contacte o administrador para associar o departamento correcto.
        </p>
        {/* Debug info para facilitar diagnóstico */}
        <p className="text-[10px] text-zinc-300 mt-3 font-mono">
          chefeDe: {user?.chefeDe?.nome ?? 'null'} · departamento:{' '}
          {user?.departamento?.nome ?? 'null'}
        </p>
      </div>
    )
    ...
    <div className="space-y-6">
      ...
      <div>
        ...
        <p className="text-sm text-zinc-500 mt-0.5">
          {deptNome} · {tecnicos.length} técnico
          {tecnicos.length !== 1 ? 's' : ''}
        </p>
      </div>
      ...
      <div className="grid grid-cols-2 gap-3">
        ...
        <Card
          className={`border shadow-none ${paraAvaliar > 0 ? 'bg-purple-50 border-purple-200' : 'bg-zinc-50 border-zinc-200'}`}
        >
          <CardContent className="p-4 flex items-center gap-3">
            <ClipboardList
              className={`h-4 w-4 ${paraAvaliar > 0 ? 'text-purple-500' : 'text-zinc-400'}`}
            />
            <div>
              <p className="text-[11px] font-semibold uppercase tracking-widest text-zinc-400">
                Para avaliar
              ...
              <p
                className={`text-2xl font-bold leading-none mt-0.5 ${paraAvaliar > 0 ? 'text-purple-700' : 'text-zinc-900'}`}
              >
                {paraAvaliar}
              </p>
            </div>
      ...
      <Card className="border-zinc-200 shadow-sm overflow-hidden">
        {loading ? (
          ...
          <div className="p-16 text-center">
            ...
            <p className="text-xs text-zinc-400 mt-1">
              Verifique se existem técnicos no departamento{' '}
              <strong>{deptNome}</strong>.
            </p>
          </div>
        ) : (
          <div className="divide-y divide-zinc-100">
            {tecnicosFiltrados.map((t) => {
              const cfg = estadoCfg[t.fichaActual?.estado ?? '']
              return (
                <div
                  key={t.id}
                ...
                >
                  ...
                  <div className="min-w-0 flex-1">
                    ...
                    <p className="text-xs text-zinc-400 mt-0.5">{t.cargo}</p>
                  </div>
                  {/* Estado ficha */}
                  ...
                  <div className="flex items-center gap-1.5 shrink-0">
                    ...
                    {t.fichaActual?.estado === 'Pendente' && (
                      <Button
                        size="sm"
                        onClick={() => abrirAvaliar(t)}
                        className="h-8 px-3 rounded-lg bg-purple-600 hover:bg-purple-700 text-white text-xs gap-1.5"
                      >
      ...
      {/* ── Sheet detalhe do técnico ── */}
      <Sheet open={sheetTecnico} onOpenChange={setSheetTecnico}>
              ...
              <>
                ...
                <div className="flex items-center gap-4 p-4 rounded-xl bg-zinc-50 border border-zinc-200">
                  <Avatar className="h-14 w-14 ring-2 ring-zinc-200">
                    <AvatarImage src={tecnicoVer.avatarUrl} />
                    <AvatarFallback className="bg-zinc-200 text-zinc-700 text-sm font-bold">
                      {getInitials(tecnicoVer.nomeCompleto)}
                    </AvatarFallback>
                  </Avatar>
                  <div>
                    ...
                    <span className="inline-flex items-center gap-1 mt-1.5 text-[11px] font-semibold text-zinc-500 bg-zinc-100 px-2 py-0.5 rounded-full font-mono">
                      ...
                      {tecnicoVer.numeroMecanografico}
                    </span>
                  </div>
                </div>

                {/* Estado da ficha */}
                {tecnicoVer.fichaActual ? (
                  <div
                    className={`rounded-xl border p-3.5 flex items-center justify-between ${estadoCfg[tecnicoVer.fichaActual.estado]?.bg ?? 'bg-zinc-50 border-zinc-200'}`}
                  >
                    <div>
                      ...
                      <p className="text-sm font-semibold text-zinc-800 mt-0.5">
                        {tecnicoVer.fichaActual.periodo.nome}
                      </p>
                    </div>
                    <span
                      className={`text-[11px] font-bold px-2.5 py-1 rounded-full ${estadoCfg[tecnicoVer.fichaActual.estado]?.badge ?? 'bg-zinc-100 text-zinc-600'}`}
                    >
                      {estadoCfg[tecnicoVer.fichaActual.estado]?.label ??
                        tecnicoVer.fichaActual.estado}
                    </span>
                  </div>
                ) : (
                    ...
                    {[
                      {
                        ...
                        label: 'Email',
                        value: tecnicoVer.email,
                      },
                      {
                        icon: <Phone className="h-3.5 w-3.5" />,
                        label: 'Telefone',
                        value: tecnicoVer.telefone,
                      },
                      {
                        icon: <Phone className="h-3.5 w-3.5" />,
                        label: 'Telemóvel',
                        value: tecnicoVer.telemovel,
                      },
                    ]
                      .filter((r) => r.value)
                      .map((r, i) => (
                    ...
                    {[
                      {
                        ...
                        label: 'Nascimento',
                        value: formatDate(tecnicoVer.dataNascimento),
                      },
                      {
                        icon: <Calendar className="h-3.5 w-3.5" />,
                        label: 'Admissão',
                        value: formatDate(tecnicoVer.dataAdmissao),
                      },
                      {
                        icon: <MapPin className="h-3.5 w-3.5" />,
                        label: 'Localidade',
                        value:
                          [tecnicoVer.localidade, tecnicoVer.provincia]
                            .filter(Boolean)
                            .join(', ') || '—',
                      },
                    ].map((r, i) => (
                ...
                {/* Botão avaliar */}
                {tecnicoVer.fichaActual?.estado === 'Pendente' && (
                  <Button
                    onClick={() => {
                      setSheetTecnico(false)
                      setTimeout(() => abrirAvaliar(tecnicoVer), 150)
                    }}
                    className="w-full h-9 rounded-lg bg-purple-600 hover:bg-purple-700 text-white text-sm font-medium gap-2"
                  >
      ...
      <Sheet open={sheetAvaliar} onOpenChange={setSheetAvaliar}>
        <SheetContent className="w-full sm:max-w-lg flex flex-col gap-0 p-0 overflow-hidden">
          <SheetHeader className="px-6 py-5 border-b border-zinc-100 shrink-0">
            <div className="flex items-center gap-3">
              ...
              <div>
                ...
                <SheetDescription className="text-[12px] text-zinc-500 mt-0.5">
                  {tecnicoAvaliar?.nomeCompleto} ·{' '}
                  {tecnicoAvaliar?.fichaActual?.periodo.nome}
                </SheetDescription>
              </div>
            </div>
          </SheetHeader>
          ...
          <div className="flex-1 overflow-y-auto px-6 py-5 space-y-5">
            {tecnicoAvaliar && (
              <div className="flex items-center gap-3 p-3.5 rounded-xl bg-zinc-50 border border-zinc-200">
                <Avatar className="h-10 w-10 shrink-0">
                  <AvatarImage src={tecnicoAvaliar.avatarUrl} />
                  <AvatarFallback className="bg-zinc-100 text-zinc-600 text-xs font-semibold">
                    {getInitials(tecnicoAvaliar.nomeCompleto)}
                  </AvatarFallback>
                </Avatar>
                <div className="min-w-0 flex-1">
            ...
            {criterios.length > 0 && (
              <div className="space-y-1.5">
                ...
                <Textarea
                  ...
                  placeholder="Observações sobre o desempenho do técnico..."
            ...
            {pontuacaoMedia && (
              <div className="rounded-xl bg-purple-50 border border-purple-200 p-4 flex items-center gap-4">
          ...
          <div className="px-6 py-4 border-t border-zinc-100 shrink-0 flex gap-2.5 bg-white">
            ...
            <Button
              ...
              className="flex-1 h-9 rounded-lg bg-purple-600 hover:bg-purple-700 text-white text-sm font-medium gap-1.5"
            >
              {submitting ? (
                <>
                ...
                <>
                  ...
                  {tecnicoAvaliar?.nomeCompleto.split(' ')[0]}
                </>
              )}
            </Button>
          </div>
