...
export default function AutoAvaliacaoPageTecnico() {
  ...
  useEffect(() => {
    async function load() {
      ...
      try {
        ...
        // 2. Período activo
        const resPeriodo = await fetch('/api/periodos?activo=true&limit=1', {
          credentials: 'include',
        })
        ...
        // 3. Ficha do técnico no período activo
        ...
        if (!fichaActiva) {
          // Período existe mas o Master ainda não criou ficha para este técnico
          setSemFicha(true)
          setLoading(false)
          return
        }
        ...
        // 5. Critérios do departamento + individuais
        ...
        console.error('[AUTO_AVALIACAO]', e)
      ...
      <div className="space-y-6">
        ...
        <div className="rounded-md border border-zinc-200 bg-white p-12 text-center">
          ...
          <p className="text-xs text-zinc-400 mt-1">
            Aguarde que o director(a) abra um novo período de avaliação.
          </p>
        </div>
      </div>
    )
  ...
  // ── Sem ficha criada (período activo mas master ainda não criou ficha) ─
  if (semFicha) {
    return (
      <div className="space-y-6">
        <div>
        ...
        <div className="rounded-md border border-amber-200 bg-amber-50 p-8 text-center">
          ...
          <p className="text-xs text-amber-600 mt-1">
            O período <strong>{periodoActivo?.nome}</strong> está activo, mas o
            director(a) ainda não criou a sua ficha de avaliação. Aguarde e
      ...
      <div className="space-y-6">
        ...
        <div className="rounded-md border border-emerald-200 bg-emerald-50 p-8 text-center">
          ...
          <p className="text-sm text-emerald-600 mt-1">
            A sua avaliação foi registada. Aguarde a avaliação do chefe de
            departamento.
          </p>
  ...
  // ── Ficha não está em Pendente (já avançou no fluxo) ─────
  if (ficha && ficha.estado !== 'Pendente') {
    const estadoLabel: Record<string, string> = {
      AutoAvaliacao: 'Auto-avaliação submetida',
      AvaliadoPorChefe: 'Em avaliação pelo chefe',
    ...
    <div className="space-y-6">
      <div>
        ...
        <p className="text-sm text-zinc-500 mt-0.5">
          {ficha?.periodo.nome} · Avalie o seu desempenho em cada critério de 1
          a 5
        </p>
      </div>
        ...
        {criterios.length === 0 ? (
          <div className="rounded-md border border-zinc-200 bg-white p-8 text-center">
            ...
            <p className="text-sm text-zinc-400">
              Nenhum critério definido para o seu departamento.
            </p>
            <p className="text-xs text-zinc-300 mt-1">
              Contacte o seu director para que sejam criados critérios.
            </p>
          </div>
        ) : (
          criterios.map((c) => (
      ...
      {criterios.length > 0 && (
        <div className="space-y-1.5">
          ...
          <Textarea
            ...
            placeholder="Descreva brevemente o seu desempenho neste período..."
